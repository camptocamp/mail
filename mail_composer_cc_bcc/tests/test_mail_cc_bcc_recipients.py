...
class TestMailCcBccRecipients(TransactionCase, MailCase):
    ...
    * a Bcc header on the Bcc recipients' emails only.
    ...
    def _send(self, subject, partners_to, partners_cc, partners_bcc):
    ...
    def _send_from_followers(self, subject, partners_to, partners_cc, partners_bcc):
    ...
    def _assert_same_to_cc_headers(self, partners_to, partners_cc):
        self._assert_emails_defined()
        # To is built from a list and Cc from a joined string, so the two
        # headers are not serialized the same way: compare addresses, not text.
        for header, expected in (("msg_to", partners_to), ("msg_cc", partners_cc)):
            values = {email[header] for email in self.emails}
            self.assertEqual(
                len(values),
                1,
                f"Every email must show the same {header}, got {sorted(values)}",
            )
            addresses = sorted(email_normalize(a) for a in email_split(values.pop()))
            self.assertEqual(addresses, sorted(expected.mapped("email")))
    ...
    def test_followers_are_to_recipients(self):
        """Followers get the email even though partner_ids stays empty

        Nothing is typed in the composer's To: the recipients come from the
        record's followers, which only exist on mail.mail.recipient_ids.
        """
        partners_to = self.us1 + self.fr
