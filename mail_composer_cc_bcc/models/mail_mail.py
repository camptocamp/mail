...
class MailMail(models.Model):
    ...
    def _prepare_outgoing_list(
        ...
        # Odoo splits a notification into one mail.mail per recipient lang, so
        # self.recipient_ids only holds the recipients of this lang group, while
        # the headers must be the same on every email: gather every group.
        # self.partner_ids is not usable here: it only holds the recipients that
        # were added explicitly in the composer, and stays empty when they come
        # from the followers of the record.
        partners_cc_bcc = self.recipient_cc_ids + self.recipient_bcc_ids
        # mail_ids is restricted to base.group_system, hence the sudo
        all_recipients = self.mail_message_id.sudo().mail_ids.recipient_ids
