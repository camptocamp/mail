...
class MailMail(models.Model):
    ...
    def _prepare_outgoing_list(
        ...
        # self.recipient_ids only holds the recipients of this lang group. The
        # headers must be the same on every email, so they are built from the
        # message level recipients (self.partner_ids, from mail.message).
        partners_cc_bcc = self.recipient_cc_ids + self.recipient_bcc_ids
        partner_to = self.partner_ids - partners_cc_bcc
        email_to = format_emails(partner_to)  # partner_ids - cc - bcc
